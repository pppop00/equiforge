...
This is the **single source of truth** for the `/log-incident` flow. The per-harness shells under `.claude/commands/`, `.codex/prompts/`, and `.cursor/commands/` all delegate here. Edit this file; the shells should not drift.
...
The user's one-line description is provided as the command argument. In Claude Code it arrives as `$ARGUMENTS`; in other harnesses it is whatever text follows the slash-command invocation. Treat it as the *what* — you will write the *why*, *rule*, and *detection* yourself from evidence.

## Procedure

1. **Parse the user's description.** Short summary only — do not trust it for paths, phase ids, or root-cause claims.
...
6. **On confirm, append.** Insert the new entry **before** the trailing `## How this file is used` section. Preserve the `---` separator above each entry. Do not reorder existing entries.
