...
When the user asks for company research (any of the trigger phrases in `SKILL.md`), do **not** answer with ad-hoc web search. The harness produces an auditable HTML report + 5 PNG knowledge-map cards + DB rows that ad-hoc answers cannot. Open in order:
...
- The root `agents/` directory contains **only** Anamnesis-Research-owned briefs (`orchestrator`, the three gate agents `language_gate` / `palette_gate` / `sec_email_gate`, plus `intent_resolver` which handles the fourth P0 gate, `cross_validator`, `post_card_auditor`, and `attackers/`). It does **not** symlink or alias upstream agents — `workflow_meta.json` references them by real path (`skills_repo/er/agents/...`, `skills_repo/ep/agents/...`). The path is the audit surface.
