...
- Card 04 (CFA lens) must apply the concept named in `cfa_lens.concept_key` / `concept_name_cn` to this specific company — `company_application` cannot be a generic textbook restatement of the concept, `cfa_lens.formula` must be an actual formula (contains `=` and operators) rather than a textual claim dressed up as one, `cfa_lens.company_calculation` must plug this company's numbers into that formula (digits required on at least one line and they must match `financial_data.json`), and `different_angle_insight` must propose a measurable contrarian read. Generic-textbook prose on Card 4 = defect.
...
Open `card_slots.json` and the parallel `card_slots_worker_notes.json` for each of Cards 01–04. Slots in scope: `intro_sentence`, `company_focus_paragraph`, `industry_paragraph`, `revenue_explainer_points`, `recent_financial_highlights`, `five_year_arc.narrative`, `five_year_arc.inflection_points`, `cfa_lens.company_application`, `cfa_lens.different_angle_insight`, `cfa_lens.formula`, `cfa_lens.company_calculation`. Attack:
...
- **Card 4 CFA-lens substance** — `cfa_lens.company_application` must apply the named concept (`cfa_lens.concept_key` / `concept_name_cn`) to a *specific* company datum, not restate the textbook concept. `different_angle_insight` must propose a *measurable* contrarian read distinct from what the cover/Porter/financials cards already say. Flag if either sub-slot fails.
- **Card 4 `cfa_lens.formula` integrity** — must be an actual formula expression (an `=` sign separating an LHS identifier or symbol from an RHS containing operators like `/ × * + - %`), not a textual claim dressed up as a formula. `"经营杠杆较高"` is a claim, not a formula; `"DOL = %ΔEBIT / %ΔRevenue"` is a formula. Also attack formulas that are technically structured but mismatched to `cfa_lens.concept_key` (e.g. concept = `sustainable_growth` but formula is a DOL identity). Generic / off-concept / non-formula text = defect.
- **Card 4 `cfa_lens.company_calculation` faithfulness** — must plug *this company's* numbers into the formula above, not a peer's or a textbook example. At least one of the 1-3 lines must contain a digit. Cross-check every digit against `research/financial_data.json` (and `financial_analysis.json` where derived metrics live): a calculation citing `revenue $90B` when the source JSON says `$94.9B` is a fabricated plug. The result line should arrive at the same number a downstream reader would compute from the inputs. Mismatched, rounded-without-noting, or undocumented-source plugs = defect. Worker notes for this slot must carry a `primary_quote` from the 10-K / IR release / earnings call backing the inputs — this is the v3 authority slot, and a missing or stale quote here is a contract violation, not just "would strengthen this slot".
...
**Authority slot rule.** `cfa_lens.company_calculation` is the Card 4 authority slot per the v3 EP analyst-content contract — it **must** carry a `primary_quote` in worker notes (the math citation has to be backed by the source 10-K / IR release / earnings call), and the deterministic gate (`validate_card1_4_analytical_content`, `AUTHORITY_SLOTS = ("company_calculation",)`) enforces presence. Attack the quote's substance per the stale/generic test above. `cfa_lens.different_angle_insight` and `cfa_lens.company_application` are no longer authority slots — you may still attack them for analytical depth (recycled `variant_view`, unfalsifiable `falsifier`, textbook restatement), but if you want a `primary_quote` there, frame it as "would strengthen this slot", not as a contract violation. The pre-v3 authority slot `cfa_lens.different_angle_insight` and the deleted-in-v3 `cfa_lens.takeaway` no longer trigger contract-level findings on their own.

**Severity rule.** ≥2 Cards 1–4 slots flagged, OR Card 4 `cfa_lens.formula` is not a real formula, OR `cfa_lens.company_calculation` plugs numbers that don't match `financial_data.json` → `severity: critical`. Single weak slot → `severity: warn`. Use `attack_class: card_voice_substrate`.
